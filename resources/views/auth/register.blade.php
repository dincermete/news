...
@section('content')
    <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
        <div class="grid gap-5 lg:grid-cols-2">
            {{-- Üye ol form --}}
            <div
                class="rounded-[20px] border border-ink/10 bg-white p-6 sm:p-8"
                x-data="{ showPassword: false, showPasswordConfirm: false }"
            >
                <h1 class="font-display text-2xl font-semibold tracking-tight text-ink">Üye ol</h1>
                ...
                <a href="{{ route('auth.google.redirect', ['ref' => $ref]) }}" class="mt-6 flex w-full items-center justify-center gap-x-2.5 rounded-xl border border-ink/10 bg-paper px-4 py-3 text-sm font-semibold text-ink transition hover:bg-ink/5 active:scale-[0.98]">
                ...
                <form method="post" action="{{ route('register.store') }}" class="mt-6 space-y-4">
                    ...
                    <div>
                        <label for="phone" class="{{ $label }}">Telefon <span class="font-normal normal-case tracking-normal text-ink-3">(opsiyonel)</span></label>
                        <input
                            id="phone"
                            type="tel"
                            name="phone"
                    ...
                    <button type="submit" class="mt-2 inline-flex w-full items-center justify-center rounded-xl bg-gradient-to-b from-black to-[#363b3c] px-4 py-3.5 text-sm font-semibold text-white transition hover:scale-[1.01] active:scale-[0.98]">
                        Üye ol
                    </button>
                </form>
            </div>

            {{-- Giriş CTA + avantajlar --}}
            <div class="flex flex-col rounded-[20px] border border-ink/10 bg-white p-6 sm:p-8">
                <h2 class="font-display text-2xl font-semibold tracking-tight text-ink">Giriş yap</h2>
                <div class="mt-4 h-px w-full bg-ink/10" aria-hidden="true"></div>

                <p class="mt-6 text-sm font-medium leading-relaxed text-ink-2">
                    Zaten {{ $brand }} üyesi misiniz? Hesabınıza giriş yaparak siparişlerinize ve panelinize devam edin.
                </p>
                ...
                <a href="{{ route('login') }}" class="mt-10 inline-flex w-full items-center justify-center rounded-xl border border-ink bg-white px-4 py-3.5 text-sm font-semibold text-ink transition hover:bg-ink/5 active:scale-[0.98]">
                    Giriş yap
                </a>
            </div>
        </div>
